...
def data_load(data_file,dimention,debaseline):
    ...
    dataset_dir = "amigos_shuffled/primer_shuffled_Default/video/" + with_or_without + "_" + arousal_or_valence + "/"
    ...
    #mencetak urutan label dari HAHV, HALV, LAHV, LALV
...
def CapsNet(input_shape, n_class, routings, batch_size):
    ...
    primarycaps = layers.Reshape(target_shape=[-1, 8], name='primarycap_reshape')(conv4)
...
if __name__ == "__main__":
    for dimention in dimentions:
        for subject in subjects:
            ...
            parser.add_argument('--save_dir', default='PRIMER_result_dependent_default_Capsnet/video/sub_dependent_'+ model_version +'/') # other
            ...
            for curr_fold in range(fold):
                ...
                test_precision_fold = precision_score(np.argmax(y_test, 1), np.argmax(y_pred, 1), average='macro')
                test_recall_fold = recall_score(np.argmax(y_test, 1), np.argmax(y_pred, 1), average='macro')
                test_f1score_fold = f1_score(np.argmax(y_test, 1), np.argmax(y_pred, 1), average='macro')
