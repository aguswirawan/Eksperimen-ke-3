...
def data_load(data_file,dimention,debaseline):
    ...
    dataset_dir = "amigos_shuffled/primer_shuffled_MWMF_div/Video/"+ with_or_without + "_" + arousal_or_valence + "/"
...
def Convolution(input_shape, n_class):
    ...
    fcn.add(layers.Dense(3, activation='softmax'))
...
#subjects = ['Sub01','Sub03','Sub04','Sub06','Sub09','Sub11','Sub12','Sub13','Sub15','Sub16','Sub18','Sub19','Sub20','Sub21','Sub22']
subjects = ['Sub02','Sub05','Sub07','Sub08','Sub10','Sub14','Sub17','Sub23','Sub24']
...
if __name__ == "__main__":
    for dimention in dimentions:
        for subject in subjects:
            ...
            parser.add_argument('--save_dir', default='PRIMER_result_dependent_MWMF_div_cnn/Video/sub_dependent_'+ model_version +'/') # other
            ...
            test_precision_allfold = np.zeros(shape=[0], dtype=float)
            test_recall_allfold = np.zeros(shape=[0], dtype=float)
            ...
            for curr_fold in range(fold):
                ...
                test_precision_fold = precision_score(np.argmax(y_test, 1), np.argmax(y_pred, 1), average=None)
                test_recall_fold = recall_score(np.argmax(y_test, 1), np.argmax(y_pred, 1), average=None)
                test_f1score_fold = f1_score(np.argmax(y_test, 1), np.argmax(y_pred, 1), average=None)
                ...
                test_precision_allfold = np.append(test_precision_allfold, test_precision_fold)
                test_recall_allfold = np.append(test_recall_allfold, test_recall_fold)
                test_f1score_allfold = np.append(test_f1score_allfold, test_f1score_fold)
                loss_allfold.append(history.history['loss'][-1])

                K.clear_session()

            summary = pd.DataFrame({'fold': range(1,fold+1), 
                                    'Test accuracy': test_accuracy_allfold, 
                                    'Test precision': test_precision_allfold.reshape(-1, len(np.unique(np.argmax(y_train, 1)))).tolist(),
                                    'Test recall': test_recall_allfold.reshape(-1, len(np.unique(np.argmax(y_train, 1)))).tolist(),
                                    'Test F1': test_f1score_allfold.reshape(-1, len(np.unique(np.argmax(y_train, 1)))).tolist(),
                                    'loss': loss_allfold, 
                                    'train time': train_used_time_allfold, 
                                    'test time': test_used_time_allfold,})
